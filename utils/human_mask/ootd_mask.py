...
from ..parameter_dict import category_dict_utils
...
def get_mask_location_garment(category, model_parse: Image.Image, keypoint: dict, width=384,height=512):
...
def get_mask_location_accessory(category, model_parse: Image.Image, keypoint: dict, width=384,height=512):
    im_parse = model_parse.resize((width, height), Image.NEAREST)
    parse_array = np.array(im_parse)
    accessory_categories = {
        "shoes": [label_map["left_shoe"], label_map["right_shoe"]],
        "hat": [label_map["hat"]],
        "glasses": [label_map["sunglasses"]],
        "belt": [label_map["belt"]],
        "scarf": [label_map["scarf"]],
    }

    results = {}

    for category_name, label_ids in accessory_categories.items():
        current_accessory_mask_array = np.zeros((height, width), dtype=np.uint8)
        for label_id in label_ids:
            current_accessory_mask_array += (parse_array == label_id).astype(np.uint8)
        current_accessory_mask_array = (current_accessory_mask_array > 0).astype(np.uint8)
        processed_mask_255 = cv2.dilate(
            current_accessory_mask_array, np.ones((3, 3), np.uint8), iterations=1
        ) * 255
        filled_mask = hole_fill(processed_mask_255)
        final_mask_array = refine_mask(filled_mask) 
        binary_mask = Image.fromarray(final_mask_array.astype(np.uint8))
        gray_mask = Image.fromarray((final_mask_array / 255 * 127).astype(np.uint8))

        results[category_name] = (binary_mask, gray_mask)
    
    return results[category]

def get_mask_image(model_image, gpu_id, category):
    """
    Get the mask image for the model image.
    """
    ...
    if category == 0 or category == 1 or category == 2:
        mask, mask_gray = get_mask_location_garment(category_dict_utils[category], model_parse, keypoints)
    elif category == 3 or category == 4 or category == 5 or category == 6 or category == 7:
        mask, mask_gray = get_mask_location_accessory(category_dict_utils[category], model_parse, keypoints)
    mask = mask.resize((768, 1024), Image.NEAREST)
    mask_gray = mask_gray.resize((768, 1024), Image.NEAREST)
    white_bg = Image.new("RGB", (768, 1024), color="white")

    masked_img = Image.composite(model_img, white_bg, mask_gray)
